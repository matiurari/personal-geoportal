export default function addPanelLayer3D(viewer, daftarLayer) {
  ...
  panel.style.cssText = `
    position:absolute;top:50px;right:50px;z-index:999;background:white;
    padding:10px 14px;border-radius:6px;font-family:sans-serif;font-size:13px;
    box-shadow:0 1px 6px rgba(0,0,0,0.3);min-width:180px;color:#111;
  `;

  panel.innerHTML = `<div style="font-weight:bold;margin-bottom:8px;">Layer 3D</div>`;
  ...
  Object.entries(daftarLayer).forEach(([nama, objek]) => {
    ...
    baris.innerHTML = `<input type="checkbox" checked /><span>${nama}</span>`;
